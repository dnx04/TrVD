...
class BatchProgramClassifier(nn.Module):
    ...
    def __init__(
        ...
        self.bilstm = nn.LSTM(encode_dim, hidden_dim, num_layers=self.num_layers, bidirectional=True, batch_first=True)
        self.encoder_layer = nn.TransformerEncoderLayer(d_model=encode_dim, nhead=4)
        self.transformer_encoder = nn.TransformerEncoder(self.encoder_layer, num_layers=2)
        self.gruout2label = nn.Linear(hidden_dim * 2, label_size)
        self.transformerout2label = nn.Linear(encode_dim, label_size)
        self.dropout = nn.Dropout(0.2)

    def init_hidden(self) -> tuple[torch.Tensor, torch.Tensor] | torch.Tensor:
        if self.gpu:
